...
*/
add_action( 'wcvendors_before_dashboard', 'autusin_wrapper_before_vendor_dashboard' );
add_action( 'wcvendors_after_dashboard', 'autusin_wrapper_after_vendor_dashboard' );
function autusin_wrapper_before_vendor_dashboard(){
	echo '<div class="vendor-dashboard-wrapper">';
}

function autusin_wrapper_after_vendor_dashboard(){
	echo '</div>';
}

add_action( 'wp', 'autusin_wcvendor_hook' );
function autusin_wcvendor_hook(){
	...
	if( 'pro' !== $pro_store_header ) {
		...
		if( WCV_Vendors::is_vendor_page() ) {
			add_action( 'woocommerce_before_main_content', 'autusin_vendor_breadcrumb', 9 );
...
function autusin_vendor_breadcrumb(){
?>
...
** Add wrapper sold by
*/
add_filter( 'wcvendors_sold_by_in_loop', 'autusin_custom_soldby' );
function autusin_custom_soldby( $sold_by_label ){
