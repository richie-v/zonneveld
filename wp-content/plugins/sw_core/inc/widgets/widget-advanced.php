...
class SW_Widgets{
	...
	protected $fields = array(
			'autusin_before_widget' => 'Before Widget',
			'autusin_after_widget'  => 'After Widget',
			'autusin_before_title'  => 'Before Title',
			'autusin_after_title'   => 'After Title',
			'autusin_display_conditions' => 'Display Conditions',
	...
	public function widget_display_callback( $instance, $widget, $args ){		
		...
		if ( autusin_check_wdisplay($widget_display) == false ) return false;
	...
	public function wp_enqueue_scripts(){
		...
		if (!isset($this->_enqueue)){
			...
			wp_enqueue_script('widget-options', $this->url . 'admin/js/admin.js', false, null, false);
			$this->_enqueue = true;
		}
	}
...
*/
function autusin_check_wdisplay ($widget_display){
	$widget_display = json_decode(json_encode($widget_display), true);
					...
					if ( ($key == 'login' && $user_ID) || ($key == 'logout' && !$user_ID) ){
						...
						if (isset($widget_display['post-type'])) {
							...
							foreach ($post_type as $type) {
								...
								if($type!=AUTUSIN_PRODUCT_TYPE)
								{
									if(!empty($type) && $type==AUTUSIN_PRODUCT_DETAIL_TYPE && is_single() && get_post_type() != 'post'){
										return true;
